try {
	var w = c.width = window.innerWidth - 30, h = c.height = document.getElementsByTagName("body")[0].offsetHeight - 890, ctx = c.getContext('2d'), opts = {
		particles : 30,
		particleBaseSize : 5,
		particleAddedSize : 5,
...
Particle.prototype.reset = function() {
	...
	var speed = this.size * opts.particleSizeSpeedMultiplier, rad = opts.particleBaseRadiant + opts.particleAddedRadiant * Math.random();
...
Trail.prototype.reset = function() {
	...
	var rad = this.parent.rad + opts.trailAddedBaseRadiant + opts.trailAddedAddedRadiant * Math.random(), speed = this.size * opts.trailSizeSpeedMultiplier;
...
Trail.prototype.step = function() {++this.tick;
...
window.addEventListener('resize', function() {
	try {
		...
		h = c.height = document.getElementsByTagName("body")[0].offsetHeight - 890;
	} catch(e) {
	}
});
...
if (h <= 0) {
	...
	document.getElementsByClassName("container")[0].style.height = window.innerHeight;
	// document.getElementsByClassName("container")[0].style.clientHeight = window.innerHeight;
	// document.getElementsByClassName("container")[0].style.offsetHeight = window.innerHeight;
};
